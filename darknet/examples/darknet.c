...
extern void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen);extern void run_yolo(int argc, char **argv);
...
//        char filename[] ="/data/yoloCao/DataSet/all_hk_chip_hc/val.txt" ;
...
int main(int argc, char **argv)
{
    ...
    if (0 == strcmp(argv[1], "detector")){
        ...
        float thresh = find_float_arg(argc, argv, "-thresh", .01);
        char *outfile = find_char_arg(argc, argv, "-out", 0);
        int fullscreen = find_arg(argc, argv, "-fullscreen");
        test_detector(argv[2], argv[3], argv[4], argv[5], thresh, .01, outfile, fullscreen);
    } else {
        fprintf(stderr, "Not an option: %s\n", argv[1]);
    }
    return 0;
