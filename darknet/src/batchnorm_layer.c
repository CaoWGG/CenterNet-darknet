...
void backward_scale_l1_grad_cpu(float penal,int n, float* scales, float* scale_updates)
{
    int f;
    for(f = 0; f < n; ++f)
    {
        scale_updates[f]+=scales[f]>=0 ? -penal:penal;
        if(scales[f]==0)scale_updates[f]=0;
    }
}
...
void backward_batchnorm_layer(layer l, network net)
{
    ...
    if(net.sparse > 0){
        backward_scale_l1_grad_cpu(net.sparse,l.out_c,l.scales,l.scale_updates);
    }
