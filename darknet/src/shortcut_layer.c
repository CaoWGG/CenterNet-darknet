...
layer make_shortcut_layer(int batch, int index, int w, int h, int c, int w2, int h2, int c2 ,int adam)
{
...
void update_shortcut_layer_gpu(layer l, update_args a)
{
    ...
    if(a.adam){
        adam_update_gpu(l.scales_gpu, l.scale_updates_gpu, l.scale_m_gpu, l.scale_v_gpu, a.B1, a.B2, a.eps, decay, learning_rate, l.out_c, batch, a.t);
    }
    else{
        axpy_gpu(l.out_c, learning_rate/batch, l.scale_updates_gpu, 1, l.scales_gpu, 1);
        scal_gpu(l.out_c, momentum, l.scale_updates_gpu, 1);
    }
}
